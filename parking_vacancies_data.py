...
city_owned = chicago.get("aksk-kvfp", limit=12811, select="pin", where="property_status='Owned by City'")
...
shapefiles = cook_county.get("2yvh-uwrw", limit=613182, select="pin10, the_geom", where="municipality='Chicago'")
for idx, _ in enumerate(shapefiles):
    if "pin10" in shapefiles[idx]:
        shapefiles[idx]["pin"] = '-'.join([shapefiles[idx]["pin10"][:2], shapefiles[idx]["pin10"][2:4], shapefiles[idx]["pin10"][4:7], shapefiles[idx]["pin10"][7:10], shapefiles[idx]["pin10"][10:]])
...
city_owned_vacant_shapefiles_df = pd.merge(city_owned_vacant_df,shapefiles_df,on='pin')
city_owned_low_utility_shapefiles_df = pd.merge(city_owned_low_utility_df,shapefiles_df,on='pin')
# Outer join city-owned vacant & low-utility shapefile dataframes with on pin
city_owned_vacant_or_low_utility_shapefiles_df = pd.merge(city_owned_vacant_shapefiles_df,city_owned_low_utility_shapefiles_df,on='pin',how='outer')
